...
const INTERESTS_ENDPOINT = '/api/auth/interests/'
  ...
  useEffect(() => {
    ...
    const loadInterests = async () => {
      try {
        ...
        setInterestsError('')
        const response = await fetch(INTERESTS_ENDPOINT, { method: 'GET' })
        if (!response.ok) {
          throw new Error(
            await extractErrorMessage(
              response,
              `Could not load interests (${response.status}).`,
            ),
          )
        }
        const data = await response.json()
    ...
    <main className="page">
      <section className="card">
        ...
        <form className="form" onSubmit={handleSubmit}>
          ...
          {isLoadingInterests && <p>Loading interests...</p>}
          {!isLoadingInterests && interestsError && (
            <p className="status-error">{interestsError}</p>
          )}
          {!isLoadingInterests && !interestsError && (
...
function ProfilePage({ navigate, onLogout }) {
  ...
  const applyProfileData = useCallback((data) => {
    ...
    setSelectedInterestIds(
      Array.isArray(data?.selected_interests)
        ? data.selected_interests
            .map((interest) => interest?.option_id)
            .filter((optionId) => Number.isInteger(optionId))
        : [],
    )
  }, [])
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await extractErrorMessage(
            response,
            `Failed to load interest options (${response.status}).`,
          ),
        )
      }
      const data = await response.json()
      ...
      setInterestsError(
        error instanceof Error ? error.message : 'Could not load interest options.',
      )
    } finally {
      setIsLoadingInterests(false)
    }
  ...
  useEffect(() => {
    loadProfile()
    loadInterestOptions()
  }, [loadInterestOptions, loadProfile])
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await extractErrorMessage(
            response,
            `Could not update profile (${response.status}).`,
          ),
        )
      }
          ...
          <div className="profile-layout">
            <article className="chat-item profile-card">
              ...
              {profileStatus.message && (
                <p
                  className={
                    profileStatus.type === 'success' ? 'status-success' : 'status-error'
                  }
                >
                  {profileStatus.message}
                </p>
                ...
                profile.selected_interests.length > 0 ? (
                  <ul>
                    {profile.selected_interests.map((interest) => (
                      <li key={interest.option_id}>{interest.name}</li>
                    ))}
                  </ul>
                ) : (
